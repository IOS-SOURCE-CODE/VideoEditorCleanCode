...
class MergeVideoWithText : OutputVideoManagable, VideoCompositionInstruction, MakeTextLayerable {
    ...
    func makeVideoFrom(data:[MergeVideoInfo], textData:[TextInfo]?, completion:@escaping VideoManagerCompletion) -> Void {
        ...
        // Export to file
        let path = NSTemporaryDirectory().appending("mergedVideoWithText.mp4")
